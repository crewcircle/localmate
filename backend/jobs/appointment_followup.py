...
# Sentinel for DB lookup failures — same shape as a service_role .execute() result
# with no data, so ``.data`` is safely falsy.
_EMPTY_RESULT = type("FakeResult", (), {"data": None})()
...
async def _enqueue_sms(pool, to: str, body: str, state: str) -> tuple[str | None, bool, str | None]:
...
async def run_appointment_followup_all_clients(arq_pool=None) -> None:  # type: ignore[no-untyped-def]
    ...
    """
    # --- Skip entire run on AU public holidays (avoids wasting Claude credit) ---
    today = date.today()
    if is_au_public_holiday(today):
        logger.info("Skipping appointment followup — %s is a public holiday", today.isoformat())
        return
...
async def _process_lapsed_patient(
    ...
    # --- Skip early if no phone (avoid wasting Claude credit) ---
